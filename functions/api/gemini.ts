...
// v7 — text-only proxy + abort timer + JSON mode + bounded output.
// v6 stripped these (because we were debugging a different failure
// path) and exposed us to Cloudflare wall-time kills again. Putting
// them back is essentially free CPU-wise and prevents the worker
// from being killed mid-fetch.
const WORKER_VERSION = 'v7-2026-05-09-text-only-bounded';
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (isAbort) {
      return new Response(
        JSON.stringify({
          error: `Gemini took longer than ${ABORT_MS / 1000}s on model ${model} — try again or set GEMINI_MODEL on the worker (e.g. gemini-2.0-flash-lite).`,
